[Github repository](https://github.com/pichenettes/eurorack/tree/master/elements)

[Schematics](downloads/elements_v02.pdf)

## Firmware hacking

Elements' source code is available under the MIT licence.

The code (along with the hardware description files) can be found in the `elements` directory in our [Eurorack modules git repository](https://github.com/pichenettes/eurorack).
...
The path to the toolchain binaries must be specified in the `TOOLCHAIN_PATH` variable in `elements/makefile`.
...
```
touch elements/resources/resources.py && make -f elements/makefile resources
```
