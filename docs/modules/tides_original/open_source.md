[Github repository](https://github.com/pichenettes/eurorack/tree/master/tides)

[Schematics](downloads/tides1_v40.pdf)

## Firmware hacking

Tides' source code is available under the MIT licence.

The code (along with the hardware description files) can be found in the `tides` directory in our [Eurorack modules git repository](https://github.com/pichenettes/eurorack).
...
The path to the toolchain binaries must be specified in the `TOOLCHAIN_PATH` variable in `tides/makefile`.
...
```
make -f tides/makefile
```

If you modify lookup tables and want the big `resources.cc` file to be regenerated:

```
touch tides/resources/resources.py && make -f tides/makefile resources
```
